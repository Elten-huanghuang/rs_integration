...
- **共振磁盘** — 一种特殊的 RS 存储磁盘，盘内物品的被动效果、物品栏 Buff 与兼容资源消耗可像玩家随身携带时一样生效；支持 Wizard Terra Curios 的全部 `BuffItem` 与 Terra Equipment 的成组无限药水
...
| 勇者传 | 可选 | 共振磁盘九剑书兼容支持 |
| 圣遗物 | 可选 | 共振磁盘消耗重定向支持 |
| Wizard Terra Curios | 可选 | 共振磁盘 `BuffItem` 物品栏效果支持 |
| Terra Equipment | 可选 | 共振磁盘成组无限药水效果支持 |
...
| **更多熔炉**     | 各级熔炉（普通、爆破、烟熏模式）¹          | ✅ | ✅ | ✅ |
| **砖制熔炉**     | 砖制熔炉、砖制高炉、砖制烟熏炉²             | ✅ | ✅ | ✅ |
| **Goety**    | 死灵火盆、黑暗祭坛、诅咒笼、灵魂烛台      | ✅ | ✅ | ❌ |
| **Malum**    | 灵魂祭坛、灵魂熔炉（核心 + 组件）      | ✅ | ✅ | ❌ |
| **Eidolon**  | 工作台、坩埚、火盆               | ✅ | ✅ | ⚠️¹ |
| **禁忌与奥秘**    | 赫菲斯托斯锻炉、锻造台，炽炉          | ✅ | ✅ | ❌ |
...
| **圣遗物**      | 纵火者法杖消耗（通过共振磁盘）         | ➖ | ➖ | ➖ |
| **禁忌与奥秘**    | 护符（通过共振磁盘）              | ➖ | ➖ | ➖ |
| **Wizard Terra Curios** | 全部 `BuffItem` 物品栏 Buff（通过共振磁盘） | ➖ | ➖ | ➖ |
| **Terra Equipment** | 达到原模组数量阈值的成组无限药水（通过共振磁盘） | ➖ | ➖ | ➖ |

> ¹ 更多熔炉当前支持普通熔炉模式及爆破/烟熏增强；工厂与发电机模式暂不支持自动加工。
> ² 砖制熔炉使用其自身的原版配方开关、配方黑名单和加工时间倍率。
...
余烬炼金台配方同时支持推断布局与确定性布局：

- 将核心材料放入炼金台、aspect 催化剂放入基座底部、配方输入放入基座顶部
- 核心材料和普通输入由合成图预留，界面中隐藏的 aspect 催化剂直接从 RS 网络补充预留，并按正确槽位顺序交付
- 推断成功后缓存炼金编码，后续可确定性执行；同一种 aspect 重复使用的配方同样受支持
